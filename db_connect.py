...
def remove_user(user_id):
    ...
    if partner_id:
        ...
        # Update the partner's status to Ustatus.PARTNER_LEFT
...
def couple(current_user_id):
    ...
    # Update both users' status to Ustatus.COUPLED
...
def uncouple(user_id):
    ...
    c.execute("UPDATE users SET partner_id=NULL WHERE user_id=?", (partner_id,))
    # Update both users' status to Ustatus.IDLE
...
def reset_users_status():
    ...
    conn = sqlite3.connect('chatbot_database.db')
    c = conn.cursor()
    # Reset the status of all users to Ustatus.IDLE
