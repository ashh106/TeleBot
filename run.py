...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if db_connect.get_user_status(user_id=user_id) == UserStatus.COUPLED:
        ...
        other_user_id = db_connect.get_partner_id(user_id)
...
async def handle_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    current_user_status = db_connect.get_user_status(user_id=current_user_id)
    ...
    if current_user_status == UserStatus.PARTNER_LEFT:
        ...
        # PARTNER_LEFT)
        db_connect.set_user_status(user_id=current_user_id, new_status=UserStatus.IDLE)
        ...
        other_user = db_connect.get_partner_id(current_user_id)
...
async def start_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    db_connect.set_user_status(user_id=current_user_id, new_status=UserStatus.IN_SEARCH)
...
async def handle_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_id = update.effective_user.id
    if user_id == ADMIN_ID:
        total_users_number, paired_users_number = db_connect.retrieve_users_number()
...
async def exit_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if db_connect.get_user_status(user_id=current_user) != UserStatus.COUPLED:
        await context.bot.send_message(chat_id=current_user, text="🤖 You are not in a chat!")
        return

    other_user = db_connect.get_partner_id(current_user)
    if other_user is None:
        return

    # Perform the uncoupling
    db_connect.uncouple(user_id=current_user)
...
async def exit_then_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    """
    current_user = update.effective_user.id
    if db_connect.get_user_status(user_id=current_user) == UserStatus.IN_SEARCH:
        return await handle_already_in_search(update, context)
...
async def blocked_bot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if is_bot_blocked_by_user(update):
        ...
        if user_status == UserStatus.COUPLED:
            other_user = db_connect.get_partner_id(user_id)
        ...
        db_connect.remove_user(user_id=user_id)
